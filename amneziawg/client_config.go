...
// Endpoint returns the full network endpoint for the AmneziaWG peer.
...
func (c *PeerClientConfig) Validate() error {
	...
	// Validate PublicKey (must be non-empty and a valid AmneziaWG public key).
	if c.PublicKey == "" {
...
// ClientConfig represents the AmneziaWG client configuration.
type ClientConfig struct {
	...
	Name         string            `mapstructure:"name"`          // Name is the name of the AmneziaWG interface.
	Peer         *PeerClientConfig `mapstructure:"peer"`          // Peer is a peer configuration that the client can connect to.
	Port         uint16            `mapstructure:"port"`          // Port specifies the AmneziaWG listening port for the client.
	PrivateKey   string            `mapstructure:"private_key"`   // PrivateKey holds the AmneziaWG private key for this client.

	Obfs Obfs `mapstructure:"obfs"` // Obfs holds the client-side obfuscation parameters received from the server.
}
...
func (c *ClientConfig) Validate() error {
	...
	// Validate PrivateKey (must be non-empty and a valid AmneziaWG private key).
	if c.PrivateKey == "" {
	...
	if err := c.Obfs.Validate(); err != nil {
		return fmt.Errorf("validating obfs: %w", err)
	}
...
func (c *ClientConfig) SetForFlags(fs *pflag.FlagSet, prefix string) {
	...
	fs.StringArrayVar(&c.ExcludeAddrs, prefix+"exclude-addrs", c.ExcludeAddrs, "exclude ip addresses/subnets from the amneziawg tunnel")
