...
func NewServer(name, appDir string, cfg *ServerConfig) *Server {
	return &Server{
		...
		homeDir: filepath.Join(appDir, "amneziawg"),
		peers:   safe.NewMap[string, Peer](),
	}
}

// WithDevice sets the AmneziaWG network interface name and returns the updated Server instance.
...
func (s *Server) Type() types.ServiceType {
	return types.ServiceTypeAmneziaWG
}

// Metadata returns the service metadata of the server.
func (s *Server) Metadata() any { return s.metadata }

// IsRunning checks if the AmneziaWG interface is up and active.
func (s *Server) IsRunning() (bool, error) {
	// Executes the 'awg show' command to check the interface status.
...
// Setup prepares the AmneziaWG server service for operation.
func (s *Server) Setup(ctx context.Context) error {
	return s.Manager.Setup(ctx, func() error { //nolint:wrapcheck
		...
		// Set the server metadata including handshake-affecting obfuscation parameters.
...
// Start starts the AmneziaWG server service.
		...
		s.Go(ctx, func() error {
			for {
				...
				case <-time.After(time.Second):
					...
					// Sync peer statistics from AmneziaWG.
...
// Stop stops the AmneziaWG server service.
...
// AddPeer adds a new peer to the AmneziaWG server.
func (s *Server) AddPeer(ctx context.Context, req any) (string, any, error) {
	...
	// Executes the 'awg set' command to add the peer to the AmneziaWG interface.
	cmd := exec.CommandContext(
		ctx,
		s.execFile("awg"),
...
// HasPeer checks if a peer exists in the AmneziaWG server's peer list.
func (s *Server) HasPeer(_ context.Context, id string) (bool, error) {
	return s.peers.Exists(id), nil
}

// RemovePeer removes a peer from the AmneziaWG server.
func (s *Server) RemovePeer(ctx context.Context, id string) error {
	// Executes the 'awg set' command to remove the peer from the AmneziaWG interface.
	cmd := exec.CommandContext(
		ctx,
		s.execFile("awg"),
...
// PeersLen returns the number of peers connected to the AmneziaWG server.
func (s *Server) PeersLen() int {
	return s.peers.Len()
}

// PeerStatistics retrieves statistics for each peer connected to the AmneziaWG server.
...
// syncPeers retrieves the latest peer transfer statistics from AmneziaWG
// and updates the in-memory peer data accordingly.
func (s *Server) syncPeers(ctx context.Context) error {
	// Executes the 'awg show' command to get transfer statistics.
	cmd := exec.CommandContext(
		ctx,
		s.execFile("awg"),
