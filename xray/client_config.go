...
type OutboundClientConfig struct {
	...
	RealityFingerprint string `mapstructure:"-"` // RealityFingerprint specifies the Reality uTLS fingerprint.
}
...
// GetFlow returns the flow as a Flow type.
func (c *OutboundClientConfig) GetFlow() Flow {
...
// ClientConfig represents the Xray client configuration options.
...
func (c *ClientConfig) SetForFlags(fs *pflag.FlagSet, prefix string) {
	...
	fs.Uint16Var(&c.API.Port, prefix+"api.port", c.API.Port, "port for the xray statistics and management operations")
	fs.Uint16Var(&c.Proxy.Port, prefix+"proxy.port", c.Proxy.Port, "port for the xray socks5 proxy server")
